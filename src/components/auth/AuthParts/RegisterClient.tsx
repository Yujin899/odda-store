      ...
      >
        <div className="bg-white border border-slate-100 p-10 rounded-(--radius) shadow-2xl space-y-8 relative overflow-hidden">
          ...
          <div className="flex flex-col items-center space-y-4">
            ...
            <h1 className="text-2xl font-black uppercase tracking-tight italic">{language === 'ar' ? 'إنشاء حساب جديد' : 'Create Account'}</h1>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            <FormInput
              label={language === 'ar' ? 'الاسم بالكامل' : 'Full Name'}
              name="name"
              icon={User}
              value={formData.name}
            ...
            <Button 
              type="submit"
              disabled={isLoading}
              className="w-full h-14 bg-foreground text-background flex items-center justify-center gap-3 font-black text-[10px] uppercase tracking-[0.3em] rounded-sm shadow-xl hover:-translate-y-0.5 transition-all active:scale-[0.98] mt-2 disabled:opacity-50 disabled:cursor-not-allowed border-none"
            >
              {isLoading 
                ? (language === 'ar' ? 'جاري إنشاء الحساب...' : 'Signing Up...') 
                : (language === 'ar' ? 'إنشاء الحساب الآن' : 'Sign Up Now')}
          ...
          <div className="pt-4 text-center">
            <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest">
              ...
              <Link href="/login" className="text-primary font-black">{dict.common.login}</Link>
            </p>
          </div>
        </div>
      </motion.div>
