      ...
      >
        <div className="bg-white border border-slate-100 p-10 rounded-(--radius) shadow-2xl space-y-8 relative overflow-hidden">
          ...
          <div className="flex flex-col items-center space-y-4">
            ...
            <h1 className="text-2xl font-black uppercase tracking-tight italic">Login to Account</h1>
          </div>

          <div className="space-y-4">
            <button
              onClick={() => signIn('google', { callbackUrl: '/' })}
              className="w-full h-14 bg-white border border-slate-200 flex items-center justify-center gap-3 font-bold text-xs uppercase tracking-widest rounded-(--radius) shadow-sm hover:bg-slate-50 transition-all cursor-pointer outline-none"
            >
            ...
            <div className="relative flex items-center justify-center py-2">
              ...
              <span className="relative bg-white px-3 text-[10px] font-black uppercase tracking-widest text-slate-300">or</span>
            </div>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <button
              type="submit"
              disabled={isLoading}
              className="w-full h-14 bg-foreground text-background flex items-center justify-center gap-3 font-black text-[10px] uppercase tracking-[0.3em] rounded-(--radius) shadow-xl hover:-translate-y-0.5 transition-all active:scale-[0.98] disabled:opacity-50 disabled:cursor-not-allowed"
            >
              {isLoading ? 'Logging in...' : 'Login to Account'}
              {!isLoading && <ChevronRight className="size-4 stroke-[3px]" />}
            </button>
          </form>
          ...
          <div className="pt-4 text-center">
            <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest">
              ...
              <Link href="/register" className="text-(--primary) font-black">Register Now</Link>
            </p>
          </div>
        </div>
      </motion.div>
